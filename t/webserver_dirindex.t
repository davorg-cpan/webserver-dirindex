...
my $html = $di->render('/test/', 0);
ok defined $html, 'render() returns a value';
like $html, qr{Index of /test/}, 'render() contains correct heading';
like $html, qr{file\.txt},       'render() contains file.txt';
like $html, qr{subdir/},         'render() contains subdir/';
unlike $html, qr{a:link},        'render() without pretty uses standard CSS';

my $html_pretty = $di->render('/test/', 1);
ok defined $html_pretty, 'render(pretty) returns a value';
like $html_pretty, qr{a:link}, 'render(pretty) uses pretty CSS';
